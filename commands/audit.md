...
Final status write: set `status.json` `phase` to `"complete"`, fill `report_md` and `report_html` with the deliverable paths (relative to the working directory, e.g. `audit-output/AUDIT-REPORT-20260707.html`; leave `report_html` null if `html_report` was false), and fill `totals` with `findings`, `blockers`, `now_items`, and `overall_score` from the fix-strategist's roadmap. The dashboard's Report view loads the HTML report and shows the run's command and per-agent models once it sees `phase: "complete"`.
